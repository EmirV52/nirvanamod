...
module.exports.run = async(client, message, args)  => {
 ...
 if(!args[0]) return message.channel.send('Sunucudan Yasaklayacağın **Kullanıcıyı** Etiketlemen Lazım!')
...
if(!user) return message.channel.send('Etiketlediğin **Kullanıcı** Sunucuda Değil!')
let member = message.guild.member(user)
if(!member) return message.channel.send('Etiketlediğin **Kullanıcı** Sunucuda Değil!')
if(member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send('Etiketlediğin **Kullanıcı** Seninle Aynı/Üst Yetkide Bulunuyor.')
