...
member.roles.remove(ayarlar.Unregistered)
...
member.send("Muteliyken Sunucudan Çıktığın için Yeniden **Muted** Rolü Verildi!")
 setTimeout(function(){
    // msg.channel.send(`<@${user.id}> Muten açıldı.`)
...
data.set(`${member.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )})
  ...
  .setColor(`#f3c7e1`)
  .setDescription(`Jailden Kaçamazsın!`)
  .setTimestamp()
    member.send(wasted)
} 
...
client.on("ready", () => {
  client.channels.cache.get("780907388881731594").join();
